...
def test_calculate_ema_with_valid_data():
    ...
    window = 3
    result = calculate_ema(prices, window=window)
    expected_result = pd.Series(prices).ewm(span=window, adjust=False).mean()
    pd.testing.assert_series_equal(result, expected_result)


def test_calculate_ema_with_empty_prices():
...
def test_calculate_ema_with_window_larger_than_prices_length():
    # Test when the window is larger than the length of prices
    prices = [10, 20]
    window = 5
    result = calculate_ema(prices, window=window)
    expected_result = pd.Series(prices).ewm(span=window, adjust=False).mean()
...
def test_calculate_ema_with_default_window():
    ...
    expected_result = pd.Series(prices).ewm(span=14, adjust=False).mean()
    pd.testing.assert_series_equal(result, expected_result)


def test_calculate_ema_with_window_one():
...
def test_calculate_ema_with_float_prices():
...
def test_calculate_ema_with_negative_window():
