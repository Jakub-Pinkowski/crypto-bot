...
with patch("utils.file_utils.load_config_values", return_value=MOCK_CONFIG_VALUES):
    from indicators.momentum_indicators import calculate_cci

def test_calculate_cci_with_valid_data():
    ...
    window = 3
    result = calculate_cci(highs, lows, closes, window=window)
    ...
    expected_cci = (typical_price - tp_sma) / (0.015 * mean_deviation)

    # Ensure both result and expected_cci are the same length
    assert len(result) == len(expected_cci)

    # Check if the values are equal, but ignore NaNs at the start
    pd.testing.assert_series_equal(result, expected_cci, check_like=True)


def test_calculate_cci_with_window_less_than_two():
...
def test_calculate_cci_with_window_larger_than_data_length():
...
def test_calculate_cci_with_valid_data_large_window():
